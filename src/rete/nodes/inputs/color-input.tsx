...
export class color_input_control extends classic_preset.Control {
  constructor(
...
export function color_input_control_component({ data }: { data: color_input_control }) {
  return (
...
export class color_node extends classic_preset.Node {
    width = 180;
    height = 180;

  constructor(initial: string = "#ffffff", change?: (updated_node_id?: string) => void) {
    super("Color Input");
    this.addControl("value", new color_input_control(initial, this.id, change));
    this.addOutput("value", new classic_preset.Output(color_socket, "Color"));
  }

  data(): { value: string } {
    const control = this.controls.value as color_input_control;
    return { value: control.color };
  }

  clone() {
    const control = this.controls.value as color_input_control;
    return new color_node(control.color, process_nodes);
  }
}