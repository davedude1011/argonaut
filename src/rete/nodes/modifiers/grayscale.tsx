...
export class grayscale_filter_node extends ClassicPreset.Node {
    width = 180;
    height = 230;

    constructor(change?: (updated_node_id?: string) => void) {
        super("Grayscale - Image Modifier");
    ...
    data(inputs: { image?: image_transport[] }): { output?: image_transport } {
        ...
        const filter = new grayscale_filter(this.id);
...
export class grayscale_filter extends filter {
    id: string;
    ...
    update(_filter: grayscale_filter): void {
        
    }

    clone(): filter {
        return new grayscale_filter(this.id);
    }
}